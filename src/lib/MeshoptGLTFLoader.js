...
const MeshoptGLTFLoader = (function () {
        ...
        for (var i = 0; i < json.extensionsUsed.length; ++i) {
          ...
          switch (extensionName) {
            ...
            case EXTENSIONS.MESHOPT_COMPRESSION:
              extensions[extensionName] = new GLTFMeshoptCompressionExtension(
                this.meshoptDecoder
              )
              break
  ...
  GLTFMeshoptCompressionExtension.prototype.decodeBufferView = function (
    extensionDef,
    buffer
  ) {
  ...
  GLTFParser.prototype.loadBufferView = function (bufferViewIndex) {
    ...
    if (
      bufferViewDef.extensions &&
      bufferViewDef.extensions[EXTENSIONS.MESHOPT_COMPRESSION]
    ) {
      var extension = this.extensions[EXTENSIONS.MESHOPT_COMPRESSION]
      var extensionDef =
        bufferViewDef.extensions[EXTENSIONS.MESHOPT_COMPRESSION]

      return this.getDependency('buffer', extensionDef.buffer).then(function (
        buffer
      ) {
      ...
      .then(function (sourceURI) {
        ...
        if (!loader && options.ddsLoader) {
          if (
            ...
            source.uri.toLowerCase().endsWith('.dds')
          ) {
            loader = options.ddsLoader
          } else if (
            source.bufferView !== undefined &&
        ...
        if (!loader && options.basisLoader) {
          if (
            source.uri !== undefined &&
            source.uri.toLowerCase().endsWith('.basis')
          ) {
            loader = options.basisLoader
          } else if (
            source.bufferView !== undefined &&
            source.mimeType == 'image/basis'
          ) {
            loader = options.basisLoader
          }
        }
