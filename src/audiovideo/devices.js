...
function getNewAudioOutputDevice(newDevices) {
  ...
  const preferredAudioOutputDeviceId = getUserSelectedOutputDeviceId(
    APP.store.getState()
  )
  ...
  if (
    ...
    availableAudioOutputDevices.find(
      (d) => d.deviceId === preferredAudioOutputDeviceId
    )
  ) {
...
function getNewVideoInputDevice(newDevices, localVideo) {
  ...
  const selectedVideoInputDeviceId = getUserSelectedCameraDeviceId(
    APP.store.getState()
  )
