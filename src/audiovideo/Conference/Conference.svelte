<script>
  ...
  function keepConferenceUpdatedRegardingTrackStore(
    ...
    return trackStore.subscribe((track_) => {
      if (track_) {
        conference
          .addTrack(track_)
          .then(() => {
            cachedLocalTracks[cacheKey] = track_
            if (onTrackAdded) onTrackAdded(track_, cacheKey)
          })
          .catch((err) => {
            console.warn(err)
          })
        ...
        if (cachedTrack) {
          conference
            ...
            .then(() => {
              delete cachedLocalTracks[cacheKey]
              if (onTrackRemoved) onTrackRemoved(track_, cacheKey)
            })
            .catch((err) => {
              console.warn(err)
            })
  ...
  keepConferenceUpdatedRegardingTrackStore(videoTrack, 'video')
